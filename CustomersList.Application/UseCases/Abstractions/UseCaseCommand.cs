...
public abstract class UseCaseCommand<TRequest, TValidator> : UseCaseBase
    where TRequest : class
    where TValidator : class, IValidator<TRequest>
{
    protected UseCaseCommand( IMapper mapper ) : base(mapper)
    {
    }

    public async Task<Result> ExecuteAsync( TRequest request, CancellationToken ct )
    {
    ...
    protected abstract Task<Result> HandleAsync( TRequest request, CancellationToken ct );
